...
for samtools_version in 1.15.1 1.14 1.13; do
    for bwa_version in 0.7.15 0.7.17; do
        docker build \
            --tag bioslimtainers/bwa-samtools:bwa-${bwa_version}_samtools-${samtools_version} \
        ...
        if [ ! -z "${DOCKER_PUSH}" ]; then
            docker push bioslimtainers/bwa-samtools:bwa-${bwa_version}_samtools-${samtools_version}
        fi
    done
done